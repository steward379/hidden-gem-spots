...
const DropImagePreview = ({ onFileUploaded, circle=false }) => {
  ...
  return (
    <div {...getRootProps()} className={`relatvie mb-5 mt-5 flex justify-center items-center border-2 border-dashed border-gray-300 
      ...
      <input {...getInputProps()} />
      {preview &&<div className="relative"><Image src={preview} alt="Preview" 
                  className={`h-32 w-32 object-cover`} width="300" height="300"/>
                  <svg className="absolute bottom-2 right-12 z-20 w-6 h-6 text-black-400 bg-white rounded-full p-1 bg-opacity-25" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
